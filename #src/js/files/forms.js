...
function select_item(select) {
	...
	if (select_type == 'input') {
		...
		const select_option_discount = select_selected_option.dataset.discount ? `<small class="_success">${select_selected_option.dataset.discount}</small>` : "";
		const select_selected_icon = select_selected_option.dataset.icon ? ' ' + select_selected_option.dataset.icon : "";
		select_type_content = '<div class="select__value"><span class="' + select_selected_icon + '">' + select_selected_text + select_option_discount + '</span></div>';
	}

	select_parent.insertAdjacentHTML('beforeend',
		'<div class="select__item">' +
		'<button ' + disabled + ' type="button" class="select__title">' + select_type_content + '</button>' +
	...
	if (select_options) {
		...
		for (let index = 0; index < select_options.length; index++) {
			...
			if (select_option_value != '') {
				...
				select_options_content = select_options_content + '<button type="button" data-value="' + select_option_value + '" class="select__option"><span class="' + select_option_icon + '">' + select_option_text + select_option_discount + '</span></button>';
			}
		}
